...
KNOWN_CVE_SIGNATURES: list[CveSignature] = [
    ...
    CveSignature(
        ...
        vendor_patterns=frozenset({"hikvision"}),
        model_patterns=frozenset({"ds-2cd2xx0", "ds-2cd2xx2", "ds-2de"}),
        binary_indicators=frozenset({"webs", "httpd", "davinci"}),
        sink_symbols=frozenset({"system", "popen", "execve"}),
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="Hikvision IP camera unauthenticated command injection via /SDK/webLanguage",
        entry_point="/SDK/webLanguage",
    ),
    # --- QNAP ---
    CveSignature(
        cve_id="CVE-2021-28799",
        vendor_patterns=frozenset({"qnap"}),
        model_patterns=frozenset({"ts-231", "ts-431", "ts-451", "ts-253", "ts-653"}),
        binary_indicators=frozenset({"authLogin.cgi", "cgi-bin"}),
        sink_symbols=frozenset(),
        vuln_type="hardcoded_cred",
        cvss_v3_score=9.8,
        description="QNAP HBS 3 hardcoded credentials allowing unauthorized access",
        entry_point="authLogin.cgi",
    ),
    CveSignature(
        ...
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="QNAP Photo Station SQL injection leading to RCE",
        entry_point="photo_station",
    ),
    # --- MikroTik ---
    CveSignature(
        ...
        cvss_v3_score=9.1,
        description="MikroTik RouterOS Winbox directory traversal + credential leak",
        entry_point="winbox:8291",
    ),
    # --- Ubiquiti ---
    CveSignature(
        cve_id="CVE-2019-12727",
        vendor_patterns=frozenset({"ubiquiti", "ui"}),
        model_patterns=frozenset({"airos", "nanostation", "litebeam", "powerbeam"}),
        binary_indicators=frozenset({"lighttpd", "httpd"}),
        sink_symbols=frozenset({"system", "popen"}),
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="Ubiquiti airOS command injection via HTTP POST request",
        entry_point="login.cgi",
    ),
    # --- Tenda ---
    CveSignature(
        cve_id="CVE-2020-10987",
        vendor_patterns=frozenset({"tenda"}),
        model_patterns=frozenset({"ac15", "ac1900", "ac10", "ac6", "ac9", "ac18"}),
        binary_indicators=frozenset({"httpd", "goform"}),
        sink_symbols=frozenset({"system", "popen", "execve", "doSystemCmd"}),
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="Tenda AC15 command injection via goform/setSysTimeCfg",
        entry_point="goform/setSysTimeCfg",
    ),
    CveSignature(
        cve_id="CVE-2022-26210",
        vendor_patterns=frozenset({"tenda"}),
        model_patterns=frozenset({"ac6v2", "ac6", "ac1206", "ac10", "ac15"}),
        binary_indicators=frozenset({"httpd"}),
        sink_symbols=frozenset({"system", "popen", "execve"}),
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="Tenda AC6V2 stack overflow and command injection via formSetFirewallCfg",
        entry_point="goform/SetFirewallCfg",
    ),
    # --- Synology ---
    CveSignature(
        ...
        vuln_type="auth_bypass",
        cvss_v3_score=9.8,
        description="Synology DiskStation Manager auth bypass via crafted HTTP request",
        entry_point="/webapi/auth.cgi",
    ),
    # --- Belkin ---
    CveSignature(
        ...
        vuln_type="cmd_injection",
        cvss_v3_score=8.8,
        description="Belkin N600/N750 command injection via HTTP request parameters",
        entry_point="setup.cgi",
    ),
    # --- TRENDnet ---
    CveSignature(
        ...
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="TRENDnet TEW-827DRU command injection via apply.cgi",
        entry_point="apply.cgi",
    ),
    # --- D-Link additional ---
    CveSignature(
        cve_id="CVE-2019-16057",
        vendor_patterns=frozenset({"dlink", "d-link"}),
        model_patterns=frozenset({"dsl-2640b", "dsl-2750b", "dsl-2730b"}),
        binary_indicators=frozenset({"mini_httpd", "httpd"}),
        sink_symbols=frozenset({"system", "popen"}),
        vuln_type="cmd_injection",
        cvss_v3_score=9.8,
        description="D-Link DSL-2640B login page command injection",
        entry_point="login.cgi",
    ),
    # --- NETGEAR additional ---
    CveSignature(
        ...
        vuln_type="buffer_overflow",
        cvss_v3_score=8.8,
        description="NETGEAR multiple router UPnP pre-auth buffer overflow",
        entry_point="upnpd:1900",
    ),
]
