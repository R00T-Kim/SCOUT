...
def write_analyst_report_v2_viewer(
    ...
    doc = "\n".join(
        [
            ...
            "      ['priority', 'score', 'source', 'path', 'families', 'next_step'].forEach(function(label) {",
            ...
            "        const steps = Array.isArray(item.analyst_next_steps) ? item.analyst_next_steps.map(asText).filter(Boolean) : [];",
