...
def write_analyst_report_v2_viewer(
    ...
    doc = "\n".join(
        [
            ...
            "    body {",
            "      background: linear-gradient(135deg, #150f23 0%, #1f1633 40%, #1a1330 100%);",
            "      background-attachment: fixed;",
            "      color: var(--ink);",
            "      font-family: 'Inter Variable', 'Inter', system-ui, -apple-system, 'Segoe UI', Helvetica, Arial, sans-serif;",
            "      font-size: 14px; line-height: 1.6;",
            "      -webkit-font-smoothing: antialiased;",
            "      min-height: 100vh;",
            "    }",
            "    pre, code, .mono { font-family: 'Berkeley Mono', 'SF Mono', 'Cascadia Code', 'Fira Code', 'JetBrains Mono', Consolas, monospace; font-size: 0.82rem; }",
            "    body::before {",
            "      content: ''; position: fixed; inset: 0; z-index: -1; pointer-events: none;",
            "      background:",
            "        radial-gradient(800px circle at 10% 20%, rgba(113,112,255,0.20) 0%, transparent 45%),",
            "        radial-gradient(600px circle at 85% 15%, rgba(192,132,252,0.18) 0%, transparent 45%),",
            "        radial-gradient(700px circle at 50% 80%, rgba(106,95,193,0.12) 0%, transparent 45%),",
            "        radial-gradient(500px circle at 95% 70%, rgba(194,239,78,0.06) 0%, transparent 40%);",
            "    }",
            "    [data-theme='light'] body { background: #f0f2f5; }",
            "    [data-theme='light'] body::before { background: radial-gradient(800px circle at 20% 30%, rgba(94,106,210,0.08) 0%, transparent 50%), radial-gradient(600px circle at 75% 15%, rgba(168,85,247,0.06) 0%, transparent 50%); }",
            ...
            "      background: rgba(21,15,35,0.80); backdrop-filter: blur(16px) saturate(180%); -webkit-backdrop-filter: blur(16px) saturate(180%);",
            "      border-bottom: 1px solid rgba(255,255,255,0.125); }",
            "    .top-bar::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 1px;",
            "      background: linear-gradient(90deg, transparent 5%, var(--accent) 30%, var(--ipc-purple) 50%, var(--accent) 70%, transparent 95%); opacity: 0.5; }",
            ...
            "    .logo { font-size: 1.1rem; font-weight: 700; letter-spacing: 0.15em; color: var(--accent); text-transform: uppercase; }",
            ...
            "    .top-bar .search-bar input:focus { border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-glow), 0 0 20px rgba(113,112,255,0.15); }",
            ...
            "      border-radius: 12px; backdrop-filter: blur(18px) saturate(180%); -webkit-backdrop-filter: blur(18px) saturate(180%);",
            "      box-shadow: 0 8px 32px rgba(0,0,0,0.3); overflow: hidden; margin-bottom: 20px;",
            "      transition: all 0.3s ease; position: relative; }",
            "    .card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px;",
            "      background: linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent); }",
            "    .card:hover { background: rgba(255,255,255,0.07); box-shadow: 0 12px 48px rgba(0,0,0,0.4); border-color: rgba(255,255,255,0.15); transform: translateY(-1px); }",
            ...
            "    pre { background: rgba(21,15,35,0.6); border: 1px solid var(--glass-border); border-radius: var(--radius-sm); padding: 20px; overflow-x: auto; font-size: 0.78rem; line-height: 1.7; color: var(--ink-secondary); backdrop-filter: blur(8px); }",
            "",
            ...
            "    tooltip.style.cssText = 'position:absolute;pointer-events:none;background:rgba(17,25,40,0.95);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:8px 12px;font-size:0.75rem;color:#f1f5f9;display:none;z-index:10;max-width:400px;word-break:break-word;';",
            ...
            "          var connEdges = edges.filter(function(ed) { return ed.source === node || ed.target === node; });",
            "          var connCount = connEdges.length;",
            "          var connHtml = '<div style=\"font-weight:700;margin-bottom:4px\">' + node.label + '</div>';",
            "          connHtml += '<div style=\"color:' + (colorMap[node.type]||'#b0bec5') + ';margin-bottom:4px\">' + node.type + '</div>';",
            "          connHtml += '<div style=\"color:#b0bec5;margin-bottom:4px\">' + connCount + ' connections</div>';",
            "          if (connCount > 0) {",
            "            connHtml += '<div style=\"border-top:1px solid rgba(255,255,255,0.1);padding-top:4px;margin-top:2px;font-size:0.7rem\">';",
            "            var shown = 0;",
            "            connEdges.forEach(function(ed) {",
            "              if (shown >= 8) return;",
            "              var peer = (ed.source === node) ? ed.target : ed.source;",
            "              var dir = (ed.source === node) ? '\\u2192' : '\\u2190';",
            "              var etype = ed.type || '';",
            "              var etypeStr = etype ? ' <span style=\"color:#c084fc\">[' + etype + ']</span>' : '';",
            "              connHtml += '<div style=\"color:#e2e8f0;white-space:nowrap\">' + dir + ' ' + (peer.label||'?') + etypeStr + '</div>';",
            "              shown++;",
            "            });",
            "            if (connCount > 8) connHtml += '<div style=\"color:#94a3b8\">... +' + (connCount-8) + ' more</div>';",
            "            connHtml += '</div>';",
            "          }",
            "          tooltip.innerHTML = connHtml;",
            "        } else {",
