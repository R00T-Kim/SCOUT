...
def write_analyst_report_v2_viewer(
    ...
    doc = "\n".join(
        [
            ...
            "      ['priority', 'score', 'source', 'path', 'families', 'attack', 'impact', 'next_step'].forEach(function(label) {",
            ...
            "        tdAttack.textContent = asText(item.attack_hypothesis) || '(none)';",
            "        tr.appendChild(tdAttack);",
            "",
            "        const tdImpact = document.createElement('td');",
            "        const impacts = Array.isArray(item.expected_impact) ? item.expected_impact.map(asText).filter(Boolean) : [];",
            "        tdImpact.textContent = impacts.length ? impacts[0] : '(none)';",
            "        tr.appendChild(tdImpact);",
            "",
            "        const tdNext = document.createElement('td');",
            "        const steps = Array.isArray(item.validation_plan)",
            "          ? item.validation_plan.map(asText).filter(Boolean)",
            "          : (Array.isArray(item.analyst_next_steps) ? item.analyst_next_steps.map(asText).filter(Boolean) : []);",
            "        tdNext.textContent = steps.length ? steps[0] : '(none)';",
