...
def _run_record(
    index: dict[str, Any], pair_id: str, side: str
) -> dict[str, Any] | None:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Score pair-eval runs into summary/ROC outputs."
    )
    ...
    for pair in pairs:
        for side_name in ("vulnerable", "patched"):
            ...
            findings_payload = (
                _load_json(run_dir / "stages" / "findings" / "findings.json")
                if run_dir
                else None
            )
            cve_payload = (
                _load_json(run_dir / "stages" / "cve_scan" / "cve_matches.json")
                if run_dir
                else None
            )
            ...
            if row.get("status") == "success":
                extraction_status = (
                    extraction_status if extraction_status != "missing" else "ok"
                )
                inventory_status = (
                    inventory_status if inventory_status != "missing" else "sufficient"
                )
            elif row.get("status") == "partial":
                extraction_status = (
                    extraction_status if extraction_status != "missing" else "partial"
                )
                inventory_status = (
                    inventory_status
            ...
            gt = determine_ground_truth(
                side_name,
                status=row.get("status") or "",
                extraction_status=extraction_status,
                target_hit=bool(cve_hits),
            )
            finding_rows.append(
                {
        ...
        vuln = next(
            (
                ...
                if r["pair_id"] == pair.pair_id and r["side"] == "vulnerable"
            ),
            None,
        )
        patched = next(
            (
                r
                for r in finding_rows
                if r["pair_id"] == pair.pair_id and r["side"] == "patched"
            ),
            None,
        )
            ...
            pair_rows.append(
                {
                    ...
                    "model": pair.model,
                    "cve_id": pair.cve_id,
                    "vulnerable_status": vuln["run_status"] if vuln else "missing",
                    "patched_status": patched["run_status"] if patched else "missing",
                    "vulnerable_ground_truth": vuln["ground_truth"] if vuln else "",
                    "patched_ground_truth": patched["ground_truth"] if patched else "",
                    "recall_hit": "",
                    "fp_hit": "",
                }
            )
        ...
        pair_rows.append(
            {
                ...
                "recall_hit": (
                    1
                    if vuln["ground_truth"] == "tp"
                    else (0 if vuln["ground_truth"] == "fn" else "")
                ),
                "fp_hit": (
                    1
                    if patched["ground_truth"] == "fp"
                    else (0 if patched["ground_truth"] == "tn" else "")
                ),
            }
        )
    ...
    resolved_vuln = [
        r
        for r in finding_rows
        if r["side"] == "vulnerable" and r["ground_truth"] in {"tp", "fn"}
    ]
    resolved_patch = [
        r
        for r in finding_rows
        if r["side"] == "patched" and r["ground_truth"] in {"fp", "tn"}
    ]
    ...
    (results_dir / "pair_eval_summary.json").write_text(
    ...
    for tier, metrics in tier_metrics.items():
        report_lines.append(
            f"| {tier} | {metrics.get('tp',0)} | {metrics.get('fp',0)} | {metrics.get('fn',0)} | {metrics.get('tn',0)} | {metrics.get('excluded',0)} |"
        )
    report_lines.extend(
        [
            "",
            "## Pair outcomes",
            "",
            "| pair_id | vuln | patched | recall_hit | fp_hit |",
            "|---|---|---|---:|---:|",
        ]
    )
    for row in pair_rows:
        report_lines.append(
            f"| {row['pair_id']} | {row['vulnerable_ground_truth']} | {row['patched_ground_truth']} | {row['recall_hit']} | {row['fp_hit']} |"
        )
    ...
    print(
        json.dumps(
            {"pairs": len(pairs), "recall": recall, "fpr": fpr}, ensure_ascii=False
        )
    )
    return 0
